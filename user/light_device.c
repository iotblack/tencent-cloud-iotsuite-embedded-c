...
void _light_on_message_received(tc_iot_message_data* md) {
    ...
    if (field_index <= 0 ) {
        ...
        reported_start = (char *)message->payload + json_token[field_index].start;
    ...
    if (field_index <= 0 ) {
        ...
        desired_start = (char *)message->payload + json_token[field_index].start;
...
void light_demo(void *pvParameter) {
    ...
    tc_iot_hal_printf("Shadow task exited.\n");
    vTaskDelete(NULL);
}
